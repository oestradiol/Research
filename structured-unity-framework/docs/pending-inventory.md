# Pending Inventory

This file records the queued later work after the New Zealand monograph-baseline release was completed on `main`. For the current package state, use `project-status.md`. For the shortest live sequencing surface, use `current-execution-order.md`. For the canonical long-range phase ladder, use [monograph/CLOSURE_PROGRAM.md](monograph/CLOSURE_PROGRAM.md). For the compressed future-facing summary, use `../ROADMAP.md`.

## A. No remaining publication or active-core blockers

The hosted `v1.5.0` release now has:

- a public remote at `https://github.com/oestradiol/Research`
- hosted README and citation URLs for the umbrella repo and public packages
- changelog entries grounded to hosted commit and tag surfaces
- root changelog preserved in `../CHANGELOG.md`

Current `main` is aligned with the hosted `v1.5.0` release for the public package and now has:

- a `38`-event New Zealand route with a `29`-event main interval
- a New Zealand monograph baseline inside the locked windows
- a **`20`-event Taiwan comparator** (threshold met)
- a retained locked payoff sentence
- a tooling-ready read-only validation layer for the stable active-core public surfaces

No unresolved publication blocker or active-core repo-completion blocker remains inside the current repository state.

## B. Completed on `main`

1. `completed` Baseline status drift removed from the active SUF surfaces.
   Why it mattered: hosted-launch completion and active empirical work needed to stop conflicting across the public status layer.

2. `completed` New Zealand hardening beyond the `35`/`27` baseline.
   Why it mattered: the late-transition DPMC authority and review layer needed to become live route material rather than held discovery residue.

3. `completed` Taiwan comparator hardening beyond the original starter tranche.
   Why it mattered: the comparator needed to stop reading as a minimal command-centre pilot.

4. `completed` Interpretation and defense closeout for the active core.
   Why it mattered: the locked payoff sentence needed to be pressure-tested and either retained or revised explicitly.

5. `completed` Tooling-ready read-only validation for the stable active-core public surfaces.
   Why it mattered: the docs-first package needed a stronger deterministic check layer before later expansion.

6. `completed` `v1.2` Taiwan completion.
   Why it mattered: the Taiwan comparator needed to reach threshold rather than remaining a bounded starter pilot.

7. `completed` Taiwan ledger extended from 15 to 20 events for threshold.
   Why it mattered: the threshold required 20+ events; five additional February-March sources coded.

8. `completed` `v1.3` Australia federal comparator completion (18 events).
   Why it mattered: Phase 2 required 18-22 event federal comparator; AHPPC + National Cabinet dual-layer architecture now visible; federal-only scope maintained.

9. `completed` `v1.4` bounded pandemic-governance closure and monograph structure (76 events, 3-case synthesis).
   Why it mattered: Three-case comparative closure now documented; patterns that travel vs. case-specific patterns explicitly stated; rival-framework positioning added; monograph TOC and chapter structure established.

## C. Long-range phase ladder

```
G[v1.2]{Taiwan-completion}+D+{completed}
G[v1.3]{Australia-federal-comparator}+D+{completed}
G[v1.4]{pandemic-governance-closure}+D+{completed}
G[v1.5]{analysis-ready-tooling}+N>{structured-exports,validators}
G[v1.6]{research-ops-tooling}+N>{maintenance-helpers-post-pain}
```

## C.1 Empirical thickening plan for locked payoff evolution

The locked payoff sentence evolves through phases as the empirical base thickens:

| Phase | Empirical Base | Target Size | Payoff Evolution | Gating Criteria |
|---|---|---|---|---|
| **Current** | NZ (38) + Taiwan (15) | 53 events | Bounded: "NZ case demonstrates..." | ✓ Hosted v1.2.0; threshold met |
| **Phase 2** | + Australia federal (18 events) | +18 events | Comparative: "Cross-case suggests..." | ✅ **COMPLETE** — 18-event federal ledger; AHPPC + National Cabinet architecture visible |
| **Phase 3** | NZ + Taiwan + Australia synthesis | 76 events | Bounded closure: "Patterns that travel..." | ✅ **COMPLETE** — 3-case synthesis documented; rival-framework positioning; monograph structure |
| **Phase 3** | + non-pandemic case | +15-25 events | Domain-spanning: "Pattern holds across perturbation types..." | Case from different domain (e.g., natural disaster, cyber incident); `12-20` events |
| **Phase 4** | + 2+ additional comparators | +30-50 events | Generalized: "Framework provides..." | 4+ cases with consistent I/C/L/U patterns; stable measurement across cases |

### Phase 2: Australia federal comparator — requirements before start

- [x] Confirm Australia official source availability (AHPPC chronology + National Cabinet transcripts available)
- [x] Define admission rule for federal vs. state-level coordination (federal-only scope per `australia-comparator-design.md`)
- [x] Pre-commit to `18-22` event target before coding begins (anti-scope-creep target set)
- [x] Draft Australia-specific coding protocol amendments (default NZ/Taiwan protocol applies)
- [x] Add Australia to `../ROADMAP.md` as active phase (marked as 🔄 active)

**Phase 2 status:** ✅ **COMPLETE** — 18-event federal comparator ledger established; AHPPC + National Cabinet dual-layer architecture visible; 20 archive-clean sources; federal-only scope maintained.

### Weakening conditions (from CONTRIBUTION_AND_POSITIONING.md)

Revise or weaken locked payoff if:
- **Boundary changes**: Taiwan/Australia show public-information as consistently downstream
- **Coding fragility**: Review shows dependence on ≤5 fragile judgment calls
- **Comparator pressure**: Null/alternative-framework comparison leaves executive-command-only reading equally good
- **Route-specificity**: Later work shows sentence is NZ-specific requiring narrower wording
- **Alternative framework victory**: Direct comparison yields better explanation without SUF vocabulary

## D. Still deferred after the long-range phase ladder

```
Q[public-doc-rewrite]{blocked:read-only-posture}
Q[additional-routes]{blocked:discipline>dilution-risk}
Q[html-mirror]{blocked:optional-convenience}
Q[atlas-growth]{blocked:breadth≠validation}
```

## E. Current watchpoints

```
W[Taiwan-wording]{threshold-met:20-events}
W[knowledge-scope]{SUF-role:foregrounded-only}
W[validator-confidence]{phrase-check≠human-judgment}
W[interface-drift]{bridge-layer≠peer-theory-layer}
```

## F. Long-horizon frontier branches — REJECTED

```
Q[consciousness-branch]{REJECTED:frontier-stack-purged}
Q[simulation-branch]{REJECTED:frontier-stack-purged}
Q[AGI-branch]{REJECTED:frontier-stack-purged}
```

All speculative frontier extensions (quantum-relational ontology/consciousness, computational simulation, AGI) have been **explicitly rejected** and removed from the framework trajectory. These aspirational branches created a "false sense of rigor" — elaborate governance apparatus around underdetermined target concepts beyond SUF's empirical base.

SUF remains strictly bounded to states/societies institutional coordination with demonstrated empirical grounding.

## G. Archived nodes — REOPEN REQUIRES JUSTIFICATION

```
A[ai-coordination]{archived:2026-04-17|reopen-requires:monograph-complete+500wd-justification}
```

AI coordination node archived per D6 decision (2026-04-17). Adjacent to rejected AGI frontier; trend-chasing risk high. Legitimate interest preserved but deferred to post-monograph phase.

**Reopen criteria:**
- Monograph structure achieved (Phase 4 complete)
- 500-word explicit justification addressing:
  - Fit within "states/societies institutional coordination" scope
  - SUF methodological value-add beyond existing literature
  - Anti-anthropomorphism discipline for this domain
  - Resource availability

## Status

`phase-aligned later-work inventory`
