# How to Read the Framework

**Full file map:** `docs/INDEX.md`

## 1. Who this document is for

Readers who want a clear path through the package without overload: newcomers, theory-first readers, route-first readers, and atlas-first readers.

## 2. Four reader paths

### Newcomer path

1. `README.md`
2. `docs/how-to-read-the-framework.md`
3. `docs/framework-overview.md`
4. `docs/claims-and-boundaries.md`
5. `framework/framework-interface.md`

### Theory-first path

1. `README.md`
2. `docs/framework-overview.md`
3. `docs/claims-and-boundaries.md`
4. `framework/framework-interface.md`
5. `framework/structural-phenomenology.md`
6. `framework/structural-phenomenology-downstream-role.md`
7. `framework/informational-awareness-framework.md`
8. `framework/iaf-downstream-role.md`
9. `framework/unity-dynamics-framework.md`
10. `framework/research-program.md`
11. `framework/literature-guide.md`

### Demonstrated-route path

1. `README.md`
2. `docs/claims-and-boundaries.md`
3. `applications/README.md`
4. `applications/demonstrated-routes/README.md`
5. `applications/demonstrated-routes/states-and-societies/institutional-coordination-under-perturbation/new-zealand-pandemic-case.md`
6. `applications/demonstrated-routes/states-and-societies/institutional-coordination-under-perturbation/bounded-gain-against-simpler-readings.md`
7. `framework/framework-interface.md` and `framework/research-program.md`

### Atlas-first path

1. `README.md`
2. `applications/README.md`
3. `applications/research-map/README.md`
4. one cluster gateway of interest
5. one or two node files of interest
6. return to `docs/claims-and-boundaries.md`, `framework/framework-interface.md`, and `framework/research-program.md`

## 3. Live sequencing note

If the question is not how to read the package but what the next active workstream is, use `docs/current-execution-order.md` together with `docs/project-status.md` and `ROADMAP.md`.

## 4. What not to assume too early

- that one demonstrated route equals the whole framework
- that `unity-like` implies strong consciousness claims
- that metrics are identical with experience
- that IAF is already a finished technical layer rather than a bounded modeling vocabulary
- that UDF already has objectively settled measurements or high-fit predictive closure

## 5. How the layers relate

- **Structural Phenomenology:** epistemic ground
- **Informational Awareness Framework:** modeling vocabulary and bridge layer for distinction, reporting, feedback, and recursive adjustment
- **Unity Dynamics Framework:** integration, coherence, latency, and unity-like organization at explicit scales and windows; the most technical-facing layer, but still route-local and underdetermined
- **Framework Interface:** bridge/control layer that keeps claim types, weak naturalization, and scope discipline explicit
