# Institutional Coordination under Perturbation

## Parent path

[Applications Atlas](../../../README.md) -> [Demonstrated Routes](../../README.md) -> [States and Societies](../README.md)

## Why this subdomain is active now

This is the clearest current public route for turning SUF into a research-ready institutional case package. It keeps the object bounded, keeps timing central, and makes `I`, `C`, `L`, and `U` legible without pretending the framework is already universal.

The route now includes a `30`-event seed ledger, a `23`-event main perturbation interval, first bounded readouts, a route-local estimator implementation, and a stabilized direct-source layer for the current DPMC / Police implementation-chain events under a live-plus-fixed-archive admission rule.

The strongest current bounded gain is that public-information coordination remains structurally central even after the route thickens with implementation-side material rather than only headline decisions.

## Current route set

- [New Zealand Pandemic Case](new-zealand-pandemic-case.md)
- [Research Design](research-design.md)
- [Source Corpus and Coding Protocol](source-corpus-and-coding-protocol.md)
- [Official Corpus Inventory](official-corpus-inventory.md)
- [Source Discovery Log](source-discovery-log.md)
- [Event Ledger Seed](event-ledger-seed.md)
- [Estimator Implementation](estimator-implementation.md)
- [First-Pass Seed Readout](first-pass-seed-readout.md)
- [First-Pass Dependency Graph](first-pass-dependency-graph.md)
- [First-Pass I Summary](first-pass-i-summary.md)
- [First-Pass C Summary](first-pass-c-summary.md)
- [First-Pass L Summary](first-pass-l-summary.md)
- [First-Pass Window Comparison](first-pass-window-comparison.md)
- [First-Pass Sensitivity and Null Note](first-pass-sensitivity-and-null-note.md)
- [Comparators, Nulls, and Comparative Extension](comparators-nulls-and-comparative-extension.md)
- [Taiwan Comparator Design](taiwan-comparator-design.md)
- [Taiwan Official Corpus Inventory](taiwan-official-corpus-inventory.md)
- [Taiwan Event Ledger Seed](taiwan-event-ledger-seed.md)
- [First NZ-Taiwan Comparison Note](first-nz-taiwan-comparison-note.md)

## Relation to the atlas

- [Crisis Governance and Emergency Management](../../../research-map/01-states-public-health-and-governance/crisis-governance-and-emergency-management.md)
- [Public Administration and Executive Coordination](../../../research-map/01-states-public-health-and-governance/public-administration-and-executive-coordination.md)
- [Comparative Pandemic Response](../../../research-map/03-next-routes-and-frontiers/comparative-pandemic-response.md)

## Status

`research-ready route`
